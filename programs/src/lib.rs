...
pub mod pyro_swap {
    ...
    pub fn open_position(
        ...
        instructions::open_position::handler(ctx, amount_in, sl_bps, tp_bps, entry_price)
    }

    pub fn execute_position(ctx: Context<ExecutePosition>, current_price: u64) -> Result<()> {
    ...
    pub fn swap(ctx: Context<Swap>, amount: u64, min_amount_out: u64) -> Result<()> {
        instructions::swap::handler(ctx, amount, min_amount_out)
    }
}
